...
#include "ctrl_iface.h"
    ...
    #include "messaging/msg_bye.h" // send_msg_bye
    #include "messaging/ssl-server.h"

    #include "ap/global_var.h"
    #include "ethanol_functions/hostapd_hooks.h"

    /* inform the controller that the AP will disconnect */
    void unload_ethanol() {
      if (config.ethanol_enable == 1) {
        // send bye message
        int id = 0;
        send_msg_bye(config.server_addr, config.remote_server_port, &id, config.local_server_port);
      }
    }

    /*
     * INIT ETHANOL COMMUNICATION SERVER
     */
    void load_ethanol(struct hapd_interfaces  * interfaces){
      ...
      srand(time(NULL)); // iniciando a raiz do randomizador
	...
	wpa_printf(MSG_INFO,"Ethanol version: %s", ETHANOL_VERSION);
        ...
        wpa_printf(MSG_INFO,"Ethanol protocol disabled!");
      }
    }
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < num_bss_configs; i++) {
    ...
    *fname++ = '\0';
     ...
     */
    #ifdef ETHANOL
        wpa_printf(MSG_INFO, "Loading ethanol configuration!");

        /** connect to hooks */
        assign_func_get_wiphy( get_wiphy );
        assign_func_get_current_frequency(get_current_frequency);
        assign_func_set_current_frequency(set_current_frequency);
        assign_func_get_current_channel(get_current_channel);
        assign_func_get_secondary_ch(get_secondary_ch);
        assign_func_set_current_channel(set_current_channel);
        assign_func_get_ssid(get_ssid);

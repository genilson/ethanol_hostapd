...
// h3dema --> new command: GET_QUEUE_PARAMS (config the AP)
...
// h3dema --> new command: SET_QUEUE_PARAMS (config the AP)
...
// h3dema --> new command: GET_WMM_PARAMS (send to the station)
static int hostapd_cli_cmd_get_wmm_params(struct wpa_ctrl *ctrl,
										  int argc, char *argv[]){
	return wpa_ctrl_command(ctrl, "GET_WMM_PARAMS");
}

// h3dema --> new command: SET_WMM_PARAMS (send to the station)
